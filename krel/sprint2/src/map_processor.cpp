...
void mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr mapMsg)
{
    ...
    double roi_width = 70;  // Set desired width for the ROI
    ...
    if (roi_width > 0 && roi_height > 0) {
        ...
        double angle = robot_theta_ * 180.0 / CV_PI; // Convert radians to degrees
        ...
        int canvas_size_x = map_section_resized.cols + 100; // Add padding for black background
        int canvas_size_y = map_section_resized.rows + 100; // Add padding for black background
        cv::Mat canvas = cv::Mat::zeros(canvas_size_y, canvas_size_x, CV_8UC3);

        // Calculate the position to place the resized map section in the center of the canvas
        ...
        // Display the resized section with black background for verification
